...
from feature_extraction import texture_statistics_pyramid_OLD as texture_statistics_pyramid
...
import merge_features, reconstruct
import fwrf_fit_OLD as fwrf_fit
import fwrf_predict_OLD as fwrf_predict
...
#     if do_fitting==False and (use_pca_pyr_feats_hl or use_pca_pyr_feats_ll or do_pca_st or do_pca_bdcn):
...
#         assert(out['best_params'][1].shape[0]==voxel_index[0].shape[0])
    ...
    if do_val: 
        ...
        sys.stdout.flush()
        if len(val_stim_data.shape)>1:
            image_size = val_stim_data.shape[2:4]
        else:
            image_size = None
        _feature_extractor.init_for_fitting(image_size, models, dtype=fpX)
        partial_masks, partial_version_names = _feature_extractor.get_partial_versions()
